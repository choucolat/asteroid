...
def b_plane_offset(v_inf: np.ndarray, standoff_km: float) -> np.ndarray:
    ...
    ``v_inf`` is the (unbiased) relative approach velocity of the spacecraft
    with respect to the target body, shape ``(..., 3)``.  The offset is
    picked along ``v_inf x z_hat`` -- i.e. perpendicular to the approach
    velocity, roughly within the ecliptic plane -- which is an arbitrary but
    consistent choice of B-plane clock angle (any direction in the B-plane
    would give the same miss distance; this just fixes which side of the
    asteroid the spacecraft passes on).

    Biasing the Lambert target point by this vector, instead of targeting the
    body's exact position, produces a genuine flyby: because the target
    body's own gravity is negligible for an object this small, the resulting
    closest-approach distance is, to good approximation, ``standoff_km``
    itself -- see ``flyby_offset_closest_approach`` for a numerical check of
    that approximation.
    """
    v_inf = np.asarray(v_inf, dtype=float)
    z_hat = np.array([0.0, 0.0, 1.0])
    n1 = np.cross(v_inf, z_hat)
    n1_norm = np.linalg.norm(n1, axis=-1, keepdims=True)
    # Guard against v_inf ~ parallel to the ecliptic pole (not expected for
    # Earth / near-Earth-asteroid heliocentric transfers, but be safe).
    degenerate = n1_norm < 1e-9
    fallback = np.broadcast_to(np.array([1.0, 0.0, 0.0]), n1.shape)
    n1 = np.where(degenerate, fallback, n1)
    n1_norm = np.linalg.norm(n1, axis=-1, keepdims=True)
    n1_hat = n1 / n1_norm
    return standoff_km * n1_hat


def solve_flyby_porkchop(
    departure_ephem: SunCentredEphemeris,
    arrival_ephem: SunCentredEphemeris,
    launch_jd: np.ndarray,
    arrival_jd: np.ndarray,
    ...
    """Like ``solve_porkchop``, but targets a B-plane-biased flyby point.

    A first ("baseline") Lambert solve, targeting the asteroid's exact
    position, is used only to get the direction of the incoming relative
    velocity; the actual transfer targets that point offset by
    ``standoff_km`` in the B-plane (see ``b_plane_offset``), so the returned
    ``dv_arrival`` is the flyby encounter speed for a pass at (approximately)
    that standoff distance, not for a direct hit.
    """
    launch_jd = np.asarray(launch_jd, dtype=float)
    arrival_jd = np.asarray(arrival_jd, dtype=float)

    r_dep, v_dep = departure_ephem.state(launch_jd)
    r_arr, v_arr = arrival_ephem.state(arrival_jd)
    ...
    _, v2_baseline = lambert_universal(r1, r2, tof, mu=mu, prograde=prograde)
    v_inf_baseline = v2_baseline - v2_body

    r2_biased = r2 + b_plane_offset(v_inf_baseline, standoff_km)

    v1, v2 = lambert_universal(r1, r2_biased, tof, mu=mu, prograde=prograde)

    dv_departure = np.linalg.norm(v1 - v1_body, axis=-1)
    dv_arrival = np.linalg.norm(v2 - v2_body, axis=-1)
...
def flyby_offset_closest_approach(
    departure_ephem: SunCentredEphemeris,
    arrival_ephem: SunCentredEphemeris,
    launch_jd: float,
    arrival_jd: float,
    ...
    """Numerically verify the closest-approach distance of a biased flyby.

    Re-solves the B-plane-biased Lambert transfer for a single launch/arrival
    pair (see ``solve_flyby_porkchop``), then propagates the spacecraft's
    unperturbed heliocentric orbit and the target body's ephemeris over a
    window centred on the arrival epoch -- sized to comfortably bracket the
    true closest-approach point -- and returns the minimum separation found.

    The target body's own gravity is not modelled here (it is negligible for
    an object the size of 2024 YR4, so the unperturbed straight-through pass
    and the true gravitationally-perturbed periapsis agree to well under a
    kilometre); this function exists to confirm that ``standoff_km`` is
    actually achieved, not to model a gravity flyby.
    """
    launch_jd = float(launch_jd)
    arrival_jd = float(arrival_jd)
    tof = (arrival_jd - launch_jd) * DAY
    if tof <= 0.0:
        return float("nan")

    r1, _ = departure_ephem.state(launch_jd)
    r2, v2_body = arrival_ephem.state(arrival_jd)

    _, v2_baseline = lambert_universal(
        r1[None, :], r2[None, :], np.array([tof]), mu=mu, prograde=prograde
    )
    v_inf_baseline = v2_baseline[0] - v2_body
    r2_biased = r2 + b_plane_offset(v_inf_baseline, standoff_km)

    v1, v2 = lambert_universal(
        r1[None, :], r2_biased[None, :], np.array([tof]), mu=mu, prograde=prograde
    )
    v2 = v2[0]
    v_inf = float(np.linalg.norm(v2 - v2_body))

    # Window sized to comfortably bracket the closest-approach point: time to
    # cross several standoff distances at the relative encounter speed.
    half_window_s = max(10.0 * standoff_km / max(v_inf, 1e-6), 3600.0)
    sample_dt = np.linspace(-half_window_s, half_window_s, int(n_samples))

    r_sc, _ = kepler_propagate(r2_biased, v2, sample_dt, mu=mu)
    r_ast, _ = arrival_ephem.state(arrival_jd + sample_dt / DAY)

    sep = np.linalg.norm(r_sc - r_ast, axis=-1)
    return float(np.min(sep))


def flyby_closest_approach(
    departure_ephem: SunCentredEphemeris,
    arrival_ephem: SunCentredEphemeris,
    launch_jd: float,
    arrival_jd: float,
    ...
    """Estimate the closest heliocentric separation during the transfer arc.

    The existing Lambert formulation matches the target body's state exactly at
    the arrival epoch, so a naive endpoint scan returns zero by construction.
    To get a nontrivial flyby-style estimate, we therefore measure the minimum
    separation over the interior of the transfer span, excluding the terminal
    ...
    r1, _ = departure_ephem.state(launch_jd)
    r2, _ = arrival_ephem.state(arrival_jd)

    v1, _ = lambert_universal(
        r1[None, :],
        r2[None, :],
        np.array([tof]),
        mu=mu,
    ...
    sample_dt = np.linspace(0.0, tof, int(n_samples))
    r_sc, _ = kepler_propagate(r1, v1, sample_dt, mu=mu)
    r_ast, _ = arrival_ephem.state(launch_jd + sample_dt / DAY)
